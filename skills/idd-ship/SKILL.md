...
3.5 **Ship-time Constitution gate (§5.3.9).**
   ...
   - Parse REVIEW.code.md: `findings = tools.ship_gate.parse_review_findings(.idd/features/<id>/REVIEW.code.md)` — the parser filters out `Status: resolved` and `Status: accepted-risk` rows automatically.
