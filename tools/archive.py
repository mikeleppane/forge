...
class PromotionResult:
    ...
    Attributes:
        ...
        skipped_terms: Terms already present with an identical (normalized)
            definition; the source-features cell may have been extended.
        conflicts: Always empty on a successful return — conflicts cause
            ``ArchiveError``. Kept on the dataclass so future callers can
            surface the structured detail without re-parsing.
    """
...
def _normalize_definition(text: str) -> str:
    """Collapse internal whitespace, strip, and lower the first letter.

    Two definitions are treated as identical when their normalized forms
    match. The parser intentionally duplicates a thin slice of the
...
def _parse_repo_glossary(text: str) -> dict[str, tuple[str, str]]:
    """Return ``{term: (definition, sources_cell)}`` for an existing repo glossary."""
...
def _render_repo_glossary(rows: dict[str, tuple[str, str]]) -> str:
    """Render the repo-wide glossary file body from a ``{term: (def, sources)}`` map."""
    lines: list[str] = []
    for term in sorted(rows, key=str.lower):
        definition, sources = rows[term]
        lines.append(f"| {term} | {definition} | {sources} |")
...
def promote_domain_to_repo(repo_root: Path, feature_id: str) -> PromotionResult:
    """Merge a feature's ``DOMAIN.md`` glossary into the repo-wide glossary.

    Conflicts (duplicate term with diverging normalized definition) abort the
    promotion via ``ArchiveError`` and leave the on-disk glossary untouched.
    ...
    Returns:
        ``PromotionResult`` describing which terms were promoted and which
        were skipped (already present with identical definitions).

    Raises:
        ArchiveError: Feature ``DOMAIN.md`` missing, or any glossary conflict
            detected. The repo-wide glossary file is not written when an
    ...
    """
    domain_path = repo_root / ".forge" / "features" / feature_id / "DOMAIN.md"
    ...
    repo_rows: dict[str, tuple[str, str]] = {}
    ...
    skipped: list[str] = []
    conflicts: list[ConflictRow] = []
    next_rows: dict[str, tuple[str, str]] = dict(repo_rows)

    for term, definition in feature_rows:
        if term not in repo_rows:
            next_rows[term] = (definition, feature_id)
            promoted.append(term)
            continue
        existing_definition, existing_sources = repo_rows[term]
    ...
    if conflicts:
        names = ", ".join(c.term for c in conflicts)
        raise ArchiveError(f"glossary conflicts: {names}")
